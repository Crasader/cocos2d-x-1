...
bool GameLayer::init()
{
    ...
    this->setTag(INDEX_GRAMLAYER);
    visibleSize=Director::getInstance()->getVisibleSize();
    origin=Director::getInstance()->getVisibleOrigin();
    
  auto ps=  new ParticleSystemHelp();
    ...
    sched->schedule(schedule_selector(GameLayer::updateAll), this, INTERVAL, false );
    
    return  true;
    }

void GameLayer:: updateAll(float dt)
{
    if (timeCount==1300)
    {
    ...
    if (isUpdateAll==false)
    {
        ...
        if (timeCountCollision*INTERVAL>=0.2)
        {
...
void GameLayer::collisiontDetection()
{
    if (barrierVector.empty())
    {
        return;
    ...
    for (; iter!=barrierVector.end(); iter++)
    {
        float min=jc->sp->getPosition().x-jc->lengthX/2;
        float max=jc->sp->getPosition().x+jc->lengthX/2;
        float target1=(*iter)->sp->getPosition().x-(*iter)->lengthX/2;
        float target2=(*iter)->sp->getPosition().x+(*iter)->lengthX/2;
        
        if (min>=target1&&target2<=max)
        {
            if ((*iter)->collision(jc))
            {
                isUpdateAll=false;
            }
        }
...
void GameLayer::deleteBarrier()
{
    ...
    for (; iter!=barrierVector.end();)
    {
        if ((*iter)->sp->getPosition().x+(*iter)->lengthX/2<0)
        {
            ...
            iter=barrierVector.erase(iter);//iter会自动指向下一个
        }
        else
        {iter++;}
    }
...
void GameLayer::intiObjectOrSprite()
{
